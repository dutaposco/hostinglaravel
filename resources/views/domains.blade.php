        ...
        <div class="container mx-auto px-6 relative z-10">
            ...
            <div class="text-center mb-16 animate-fadeInUp">
                ...
                <form @submit="handleSearch" class="max-w-3xl mx-auto relative group">
                    <div class="absolute -inset-1 bg-gradient-to-r from-blue-500 to-accent rounded-2xl blur opacity-20 group-hover:opacity-40 transition duration-500"></div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <template x-for="(tld, idx) in tlds" :key="idx">
                        <div class="card p-4 border-slate-200 text-center hover:border-accent transition-colors">
                            ...
                            <p class="text-accent font-black text-xl"><span x-text="tld.price"></span><span class="text-sm font-medium text-slate-500">/thn</span></p>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    @foreach($popularDomains as $domain)
                        <div class="card p-6 border-slate-200 hover:border-accent transition-colors">
                            ...
                            <button class="w-full btn btn-outline border-slate-200 hover:border-accent hover:bg-accent hover:text-white">
                                Daftar Sekarang
                            </button>
                        </div>
                    @endforeach
            ...
            <div x-show="isSearching" class="max-w-3xl mx-auto text-center py-20">
                <div class="w-12 h-12 border-4 border-surface border-t-accent rounded-full animate-spin mx-auto mb-4"></div>
                <p class="text-slate-500 font-medium">Memeriksa ketersediaan...</p>
            </div>

            <div x-show="results !== null && !isSearching" style="display: none;" class="max-w-3xl mx-auto animate-fadeInUp mt-8">
                ...
                <div class="bg-surface-2 border border-slate-200 rounded-2xl overflow-hidden shadow-card">
                    <template x-for="(result, idx) in results" :key="idx">
                        <div :class="idx === 0 ? 'bg-white' : ''" class="p-6 flex flex-col md:flex-row items-center justify-between gap-4 border-b border-slate-200 last:border-0 hover:bg-white transition-colors">
                            <div class="flex items-center gap-4">
                                ...
                                <div>
                                    <div class="flex items-center gap-3">
                                        <p class="font-heading font-bold text-xl" :class="!result.available ? 'text-slate-400 line-through' : 'text-slate-900'" x-text="result.domain"></p>
                                        <template x-if="result.popular && result.available">
                                            <span class="bg-accent-dim text-accent text-[10px] font-black px-2 py-0.5 rounded uppercase tracking-widest border border-accent/20">Populer</span>
                                        </template>
                                    </div>
                                    <p class="text-sm text-slate-500 mt-1" x-text="result.available ? 'Tersedia! Amankan sekarang.' : 'Sudah diambil. Coba ekstensi lain.'"></p>
                                </div>
                            </div>

                            <template x-if="result.available">
                                <div class="flex items-center gap-6">
                                    <p class="font-black text-xl text-slate-900"><span x-text="result.price"></span><span class="text-sm font-medium text-slate-500 font-body">/thn</span></p>
                                    <a href="/pricing" class="btn btn-outline border-slate-200 hover:border-accent hover:bg-accent-dim">
                                        Ke Keranjang
                                    </a>
                                </div>
                            </template>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto mt-32">
                <div class="card p-8 border-slate-200 flex items-start gap-4">
                    <div class="w-12 h-12 bg-white shadow-sm rounded-xl flex items-center justify-center text-accent shrink-0">
                    ...
                    <div>
                        ...
                        <p class="text-slate-500 text-sm leading-relaxed">Kami melindungi informasi pribadi Anda dari spammer dan penipu. Privasi WHOIS menjadi standar untuk setiap pendaftaran domain.</p>
                    </div>
                </div>
                <div class="card p-8 border-slate-200 flex items-start gap-4">
                    <div class="w-12 h-12 bg-white shadow-sm rounded-xl flex items-center justify-center text-blue-400 shrink-0">
                    ...
                    <div>
                        ...
                        <p class="text-slate-500 text-sm leading-relaxed">Tanpa menunggu. Domain Anda diaktifkan seketika dan propagasi DNS dipercepat ke seluruh jaringan global kami.</p>
                    </div>
                </div>
            </div>
        </div>
